...
from beginner_msgsrv.srv import CalcSteeringAngle
...
class Turtlebot:
    ...
    def steering_angle(self, goal_pose):
        # Call calculate_steering_angle service
        rospy.wait_for_service('calculate_steering_angle')
        try:
            calculate_steering_angle = rospy.ServiceProxy('calculate_steering_angle', CalcSteeringAngle)
            resp1 = calculate_steering_angle(self.pose.position.x, goal_pose.position.x, self.pose.position.y, goal_pose.position.y)
            return resp1.angle
